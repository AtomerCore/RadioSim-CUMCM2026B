...
- 额外的本地辅助功能：可视化页、批量跑测页（自动连跑多局并汇总统计，见上文）与
  正式测试“显示真值”调试开关（演练始终可见）。
- **可视化页**：实时地图与事件回放（机器狗、干扰源真值、示向度射线、轨迹、网格；
  真值与轨迹默认显示，检测范围与示向度射线默认关闭）。点击任意点位（干扰源、
  检测点、清除点、被拒请求、机器狗）弹出详情卡片；每个干扰源可单独控制其
  检测范围、示向度射线与显隐（隐藏后以幽灵模式淡显，仍可点击恢复）；右侧栏
  全局开关批量作用于全部干扰源，与单源状态混合时复选框显示不定态；详情卡片
  随画布拖动/缩放跟随点位，点移出视野时暂时隐藏。
- **设置页**：修改未保存时切换标签会弹窗确认（保存并离开 / 放弃修改 / 留在本页），
  标签页出现未保存圆点，关闭或刷新页面前浏览器也会提醒。
- **管理接口加固**：`/api/*` 的 POST 仅接受 `Content-Type: application/json`
  （否则 415），所有请求的 Host 头须为本机回环地址（127.0.0.1 / localhost，
  否则 421），以阻断跨站表单请求与 DNS 重绑定。机器人接口（四条精确路径）行为
  不变，浏览器与常规 HTTP 客户端均天然满足以上校验。
